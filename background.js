...
chrome.storage.local.get(["threshold", "urlList"], (object) => {
...
chrome.storage.onChanged.addListener(async () => {
  ...
  if (destroyer != null) {
    let [_, newUrlList, newThreshold] = await Promise.all([chrome.alarms.clear(destroyer.name),
    chrome.storage.local.get(["urlList"]),
    chrome.storage.local.get(["threshold"])]);
    let tabs = await chrome.tabs.query({ url: newUrlList.urlList });
    if (tabs.length !== 0) {
      chrome.alarms.create("destroyer", { delayInMinutes: newThreshold.threshold });
      console.log(`Destroy in ${newThreshold.threshold} minutes`);
    }
  }
});
